 ...
 b=BeatsSimulation;
 b.load_scenario('C:\Users\Felix\code\autoCalibrationProject\config\210E.xml');
 original=b.scenario_ptr;
 ...
 disp(f);
 final=ScenarioPtr;
 final.load('C:\Users\Felix\code\autoCalibrationProject\config\210E_structure.xml');
...
final.save('C:\Users\Felix\code\autoCalibrationProject\config\210E_truncated.xml');
copy=ScenarioPtr;
copy.load('C:\Users\Felix\code\autoCalibrationProject\config\210E_truncated.xml');
nodecopy=copy.scenario.NetworkSet.network.NodeList.node;
todelete=struct('inputs','delete','outputs','delete','position','delete','node_type','delete','ATTRIBUTE',struct('delete',''));
for i=1:size(nodecopy,2)
    id =  nodecopy(1,i).ATTRIBUTE.id;
    for k=i+1:size(nodecopy,2)
        if nodecopy(1,k).ATTRIBUTE.id==id
            final.scenario.NetworkSet.network.NodeList.node(k)=todelete;
        end
    end
end    
for i=1:size(final.scenario.SensorSet.sensor,2)
    id =  final.scenario.SensorSet.sensor(1,i).ATTRIBUTE.id;
    for k=i+1:size(final.scenario.SensorSet.sensor,2)
        if final.scenario.SensorSet.sensor(1,k).ATTRIBUTE.id==id
            final.scenario.SensorSet.sensor(1,k)=todelete;
        end
    end
end    
for i=1:size(final.scenario.SensorSet.sensor,2)
